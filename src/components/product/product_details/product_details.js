...
const ProductDetails = () => {
    ...
    useEffect(() => {
        ...
        if (state === true) {
            gsap.fromTo(".product_details_container", {
        ...
        if (state === false) {
            ...
            gsap.fromTo(".product_details_container", {
                ...
                ScrollTrigger.batch(".product_details_container", {
                    onEnter: batch => {
                        batch.forEach((ele, index) => {
                            gsap.fromTo(ele.children, {yPercent:-15,xPercent:-15,opacity:0},{yPercent:0,xPercent:-2,stagger:0.1,duration:0.9,delay:`${0.3*index}`,opacity:1})
                        })
                    }
                })
        } 
    ...
    console.log("product_details state ,",state)
    const productDetails = () => {
        return (
        <>
            {state && <div className="close" onClick={() => setState(!state)}>X</div>}
